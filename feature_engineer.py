...
def prepare_input(group):
    _feature = (pl.read_csv("./input/predict-student-performance-from-game-play/train{}.csv".format(group))
      .drop(["fullscreen", "hq", "music"])
      .with_columns(columns))
    _feature = feature_engineer(_feature, group, use_extra=True, feature_suffix='')
    # sorted wrt "session_id"
    # test_feature
    _feature = time_feature(_feature)
    _feature.fillna(0, inplace=True)
    return _feature
...
labels = pd.read_csv("./input/predict-student-performance-from-game-play/train_labels.csv")
labels['session'] = labels.session_id.apply(lambda x: int(x.split('_')[0]) )
labels['q'] = labels.session_id.apply(lambda x: int(x.split('_')[-1][1:]) )

def prepare_label(q):
    q = int(q)
    _lb = pl.from_pandas(labels.loc[labels.q == q][["session", "correct"]]).sort("session")
    _lb = _lb["correct"]
    # _lb = _lb.sort("session")
    return _lb.to_pandas()